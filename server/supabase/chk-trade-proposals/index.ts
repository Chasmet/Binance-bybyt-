...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==="server_create_proposal"){
   ...
   if(!(quote>0)||quote>30.0000001)return new Response(JSON.stringify({error:"invalid_quote_amount"}),{status:400,headers});
  ...
  if(action==="report_blocked"){
   const id=String(body?.id||"").trim(); const reason=String(body?.reason||"").trim().slice(0,500);
   if(!/^[a-f0-9-]{36}$/i.test(id)||!reason)return new Response(JSON.stringify({error:"invalid_block_report"}),{status:400,headers});
   const {error}=await sb.from("chk_trade_proposals").update({result:{blocked:true,reason,checkedAt:now},updated_at:now}).eq("id",id).eq("account_fingerprint",fp).eq("status","pending"); if(error)throw error;
