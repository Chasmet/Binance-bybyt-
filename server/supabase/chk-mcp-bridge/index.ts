...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(action==="create_trade_proposal"){
    ...
    if(!["BUY","SELL"].includes(side)||!["LIMIT","MARKET"].includes(orderType)||!(quote>0)||quote>10.0000001)return out(400,{error:"invalid_order"});
  ...
  if(action==="create_cancel_proposal"){
    ...
    const replacementQuote=b?.replacementQuoteAmountUsdc==null?null:Number(b.replacementQuoteAmountUsdc);if(replacementQuote!=null&&(!(replacementQuote>0)||replacementQuote>10.0000001))return out(400,{error:"invalid_replacement_amount"});
    const {data:existing,error:ee}=await sb.from("chk_cancel_proposals").select("id,status,symbol,target_order_id,expires_at").eq("account_fingerprint",account).eq("status","pending").eq("target_order_id",target).maybeSingle();if(ee)throw ee;if(existing)return out(200,{ok:true,duplicate:true,proposal:existing});
    const expires=Math.max(5,Math.min(1440,Number(b?.expiresInMinutes||120)));
