...
const order = {type:'object',additionalProperties:false,required:['symbol','side','quoteAmountUsdc','limitPrice'],properties:{
  ...
  orderType:{type:'string',enum:['LIMIT']},quoteAmountUsdc:{type:'number',exclusiveMinimum:1,maximum:MAX_ORDER_USDC*MAX_BATCH_ORDERS},
  limitPrice:{type:'number',exclusiveMinimum:0},baseQuantity:{type:'number',exclusiveMinimum:0},rationale:{type:'string'}
}};
export const batchTools = [
  {name:'create_trade_batch',description:`Prépare TOUS les ordres LIMIT dans un lot persistant. Découpe automatiquement un montant supérieur au plafond par ordre de ${MAX_ORDER_USDC} USDC en respectant le pas Bybit (maximum 20 ordres). Attend les états individuels OPEN, PARTIALLY_FILLED, FILLED, REJECTED ou FAILED. Un résultat reportReady=false est un suivi intermédiaire, jamais un rapport final. Réutiliser le même batchId en cas de retry. Si allConfirmed=false et pending>0, continuer avec wait_trade_batch et les proposalIds retournés, sans recréer les ordres. Ne jamais annoncer le lot placé avant allConfirmed=true. Les plafonds Android restent applicables.`,
...
export async function handleBatchTool(name, args, callBridge) {
  ...
  const instruction = data.allConfirmed === true
    ? 'Tous les placements du lot sont confirmés. Rapporter chaque état réel ; OPEN ne signifie pas FILLED.'
    : data.pending > 0
